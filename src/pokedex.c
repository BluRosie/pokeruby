...
static const u8 sDexSearchTypeIds[] = {0xFF, TYPE_NORMAL, TYPE_FIGHTING, TYPE_FLYING, TYPE_POISON, TYPE_GROUND, TYPE_ROCK, TYPE_BUG, TYPE_GHOST, TYPE_STEEL, TYPE_FIRE, TYPE_WATER, TYPE_GRASS, TYPE_ELECTRIC, TYPE_PSYCHIC, TYPE_ICE, TYPE_DRAGON, TYPE_DARK, TYPE_FAIRY};
static const struct SearchOption sSearchOptions[] =
{
    { sDexSearchNameOptions,  6,  7, 10},
    { sDexSearchColorOptions,  8,  9, 11},
    { sDexSearchTypeOptions, 10, 11, NUMBER_OF_MON_TYPES},
    { sDexSearchTypeOptions, 12, 13, NUMBER_OF_MON_TYPES},
...
static void Task_OpenInfoScreenAfterMonMovement(u8 taskId)
{
    if (gSprites[gPokedexView->selectedMonSpriteId].x == X_POSITION_FOR_DESCRIPTION_PAGE
     && gSprites[gPokedexView->selectedMonSpriteId].y == Y_POSITION_FOR_DESCRIPTION_PAGE)
    {
...
static void Task_OpenSearchResults(u8 taskId)
{
    ...
    if (LoadPokedexListPage(RESULTS_DEXPAGE))
...
static void Task_PokedexResultsScreenMenu(u8 taskId)
{
    ...
    else
    {
        if (gMain.newKeys & A_BUTTON)
        {
            ...
            case 2: //LIST BOTTOM
                gPokedexView->selectedPokemon = gPokedexView->pokemonListCount - 1;
                gPokedexView->pokeBallRotation = gPokedexView->pokemonListCount * 16 + 0x40;
                ClearMonSprites();
                CreateMonSpritesAtPos(gPokedexView->selectedPokemon, 0xE);
                gMain.newKeys |= START_BUTTON;
                break;
...
static void Task_WaitForExitSearchResultsInfoScreen(u8 taskId)
{
    if (gTasks[gTasks[taskId].data[0]].isActive)
    {
        if (gPokedexView->currentPage == PAGE_DEXPAGE && !IsInfoScreenScrolling(gTasks[taskId].data[0]) && TryDoInfoScreenScroll())
            sub_808F284(&gPokedexView->pokedexList[gPokedexView->selectedPokemon], gTasks[taskId].data[0]);
    }
    else
    {
...
static void CreateMonSpritesAtPos(u16 selectedMon, u16 b)
{
    ...
    if (dexNum != 0xFFFF)
    {
        spriteId = CreatePokedexMonSprite(dexNum, X_POSITION_FOR_SCROLLING, Y_POSITION_FOR_SCROLLING);
...
static void CreateScrollingPokemonSprite(u8 direction, u16 selectedMon)
{
    ...
    case 1:
        ...
        if (unk != 0xFFFF)
        {
            spriteId = CreatePokedexMonSprite(unk, X_POSITION_FOR_SCROLLING, Y_POSITION_FOR_SCROLLING);
            gSprites[spriteId].callback = SpriteCB_PokedexListMonSprite;
...
static u16 TryDoPokedexScroll(u16 a, u16 b)
{
    ...
    //_0808E628
    r5 = gPixelsPerScrollIteration[gPokedexView->scrollSpeed / 4];
    r3 = gScrollingIterations[gPokedexView->scrollSpeed / 4];
...
static u8 TryDoInfoScreenScroll(void)
{
    ...
    if ((gMain.newKeys & DPAD_UP) && currentMon)
    {
        ...
        while (temp != 0)
        {
            temp = GetNextPosition(1, temp, 0, gPokedexView->pokemonListCount - 1);

            if (gPokedexView->pokedexList[temp].seen)
            {
                currentMon = temp;
                break;
            }
    ...
    else if ((gMain.newKeys & DPAD_DOWN) && currentMon < gPokedexView->pokemonListCount - 1)
    {
        ...
        {
            temp = GetNextPosition(0, temp, 0, gPokedexView->pokemonListCount - 1);
...
static u32 CreatePokedexMonSprite(u16 num, s16 x, s16 y)
{
    ...
    {
        if (gPokedexView->monSpriteIds[palette] == 0xFFFF)
        {
            u8 spriteId = CreateMonSpriteFromNationalDexNumber(num, x, y, palette);
            ...
            gPokedexView->monSpriteIds[palette] = spriteId;
            return spriteId;
        }
    }
    return 0xFFFF;
...
static void SpriteCB_MoveMonForInfoScreen(struct Sprite *sprite)
{
    ...
    sprite->x2 = 0;
    sprite->y2 = 0;
    if (sprite->x != X_POSITION_FOR_DESCRIPTION_PAGE || sprite->y != Y_POSITION_FOR_DESCRIPTION_PAGE)
    {
        if (sprite->x > X_POSITION_FOR_DESCRIPTION_PAGE)
            sprite->x--;
        if (sprite->x < X_POSITION_FOR_DESCRIPTION_PAGE)
            sprite->x++;

        if (sprite->y > Y_POSITION_FOR_DESCRIPTION_PAGE)
            sprite->y--;
        if (sprite->y < Y_POSITION_FOR_DESCRIPTION_PAGE)
            sprite->y++;
    }
...
static void SpriteCB_PokedexListMonSprite(struct Sprite *sprite)
{
    ...
    if (gPokedexView->currentPage != MAIN_DEXPAGE && gPokedexView->currentPage != RESULTS_DEXPAGE)
    {
        DestroySprite(sprite);
...
static void SpriteCB_Scrollbar(struct Sprite *sprite)
{
    if (gPokedexView->currentPage != 0 && gPokedexView->currentPage != 3)
        ...
        sprite->y2 = gPokedexView->selectedPokemon * CURSOR_RANGE_Y / (gPokedexView->pokemonListCount - 1);
}
...
static void Task_InitPageScreenMultistep(u8 taskId)
{
    ...
    default:
        if (!gPaletteFade.active)
        {
            ...
            gPokedexView->currentPage = PAGE_DEXPAGE;
            gPokedexView->descriptionPageNum = 0;
    ...
    case 4:
        ...
        if (sPokedexListItem->owned)
        {
            ...
            Menu_PrintText(gPokedexEntries[sPokedexListItem->dexNum].description, DESCRIPTION_X_POSITION, DESCRIPTION_Y_POSITION);
    ...
    case 5:
        if (gTasks[taskId].data[1] == 0)
        {
            gTasks[taskId].data[4] = (u16)CreateMonSpriteFromNationalDexNumber(sPokedexListItem->dexNum, X_POSITION_FOR_DESCRIPTION_PAGE, Y_POSITION_FOR_DESCRIPTION_PAGE, 0);
...
static void Task_InitAreaScreenMultistep(u8 taskId)
{
    ...
    default:
        if (!gPaletteFade.active)
        {
            gPokedexView->currentPage = AREA_DEXPAGE;
            gPokedexVBlankCB = gMain.vblankCallback;
            SetVBlankCallback(NULL);
            ResetOtherVideoRegisters(0x200);
...
static void Task_InitCryScreenMultistep(u8 taskId)
{
    ...
    default:
        if (!gPaletteFade.active)
        {
            ...
            gPokedexView->currentPage = CRY_DEXPAGE;
            gPokedexVBlankCB = gMain.vblankCallback;
            SetVBlankCallback(NULL);
            ResetOtherVideoRegisters(0x200);
    ...
    case 5:
        gTasks[taskId].data[4] =
            CreateMonSpriteFromNationalDexNumber(sPokedexListItem->dexNum, X_POSITION_FOR_CRY_PAGE, Y_POSITION_FOR_CRY_PAGE, 0);
        gSprites[gTasks[taskId].data[4]].oam.priority = 0;
...
static void Task_InitSizeScreenMultistep(u8 taskId)
{
    ...
    case 0:
        if (!gPaletteFade.active)
        {
            gPokedexView->currentPage = SIZE_DEXPAGE;
            gPokedexVBlankCB = gMain.vblankCallback;
            SetVBlankCallback(NULL);
            ResetOtherVideoRegisters(0x200);
...
static void DisplayDexForNewMon(u8 taskId)
{
    ...
    default:
        if (!gPaletteFade.active)
        {
            gPokedexVBlankCB = gMain.vblankCallback;
            SetVBlankCallback(NULL);
            ResetOtherVideoRegisters(0x100); // init all but bg0
    ...
    case 3:
        ...
        break;
    case 4:
        spriteId = CreateMonSpriteFromNationalDexNumber(dexNum, X_POSITION_FOR_DESCRIPTION_PAGE, Y_POSITION_FOR_DESCRIPTION_PAGE, 0);
        gSprites[spriteId].oam.priority = 0;
        BeginNormalPaletteFade(0xFFFFFFFF, 0, 16, 0, RGB(0, 0, 0));
        SetVBlankCallback(gPokedexVBlankCB);
...
void PrintFootprint(u16 num, u16 b, u16 c)
{
    ...
    r12 = gMonFootprint_Rayquaza;//(const u8*)sMonFootprintTable[NationalPokedexNumToSpecies(num)];
    r7 = 0;
    for (i = 0; i < 32; i++)
    {
        r3 = r12[i];
    ...
    CreateMonIcon(num, SpriteCB_PokemonIcon, 208, 68, 3, 0);
}
...
static void Task_LoadSearchMenu(u8 taskId)
{
    ...
    case 0:
        if (!gPaletteFade.active)
        {
            gPokedexView->currentPage = DEXPAGE_2;
            ResetOtherVideoRegisters(0);
...
static void Task_HandleSearchParameterInput(u8 taskId)
{
    ...
    numOptions = sSearchOptions[r1].numOptions - 1;
    if (gMain.newKeys & A_BUTTON)
    {
        sub_814ADC8();
        PlaySE(SE_PIN);
