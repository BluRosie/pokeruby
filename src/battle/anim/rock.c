...
const struct SpriteTemplate gRockPolishStreakTemplate =
{
...
const struct SpriteTemplate gRockPolishSparkleTemplate =
{
...
static void AnimPowerGemScatter(struct Sprite *sprite)
{
...
static void AnimPowerGemOrbitFast(struct Sprite *sprite)
{
    sprite->pos1.x = GetBattlerSpriteCoord(gBattleAnimAttacker, 2);
    sprite->pos1.y = GetBattlerSpriteCoord(gBattleAnimAttacker, 3);
...
static void AnimStealthRockStep(struct Sprite *sprite)
{
    if (TranslateAnimArc(sprite))
    {
        ...
        StoreSpriteCallbackInData(sprite, AnimStealthRockStep2);
    }
}
