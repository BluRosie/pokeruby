...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= 6) //status change
    {
        ...
        case STATUS_POISON:
            ...
            if ((IS_BATTLER_OF_TYPE(gEffectBank, TYPE_POISON) || IS_BATTLER_OF_TYPE(gEffectBank, TYPE_STEEL))
                && (gHitMarker & HITMARKER_IGNORE_SAFEGUARD) 
                && (primary == TRUE || certain == MOVE_EFFECT_CERTAIN))
            {
                BattleScriptPush(gBattlescriptCurrInstr + 1);
            ...
            if (IS_BATTLER_OF_TYPE(gEffectBank, TYPE_POISON))
                break;
            if (IS_BATTLER_OF_TYPE(gEffectBank, TYPE_STEEL))
                break;
            if (gBattleMons[gEffectBank].status1) 
                break;
            if (gBattleMons[gEffectBank].ability == ABILITY_IMMUNITY)
        ...
        case STATUS_BURN:
            ...
            if (IS_BATTLER_OF_TYPE(gEffectBank, TYPE_FIRE)
                && (gHitMarker & HITMARKER_IGNORE_SAFEGUARD) 
                && (primary == TRUE || certain == MOVE_EFFECT_CERTAIN))
            {
                BattleScriptPush(gBattlescriptCurrInstr + 1);
        ...
        case STATUS_FREEZE:
            ...
            if (IS_BATTLER_OF_TYPE(gEffectBank, TYPE_ICE))
                break;
            if (gBattleMons[gEffectBank].status1)
                break;
            if (noSunCanFreeze == 0)
            ...
            case MOVE_EFFECT_STEAL_ITEM:
                {
                    ...
                    if (GetBattlerSide(gBankAttacker) == B_SIDE_OPPONENT
                        && !(gBattleTypeFlags & 
                        ...
                        && gTrainerBattleOpponent != 0x400)
                    {
                        gBattlescriptCurrInstr++;
                              ...
                              | BATTLE_TYPE_LINK)) 
                             && gTrainerBattleOpponent != 0x400 
                             && (gWishFutureKnock.knockedOffPokes[side] & gBitTable[gBattlerPartyIndexes[gBankAttacker]]))
                    {
                        gBattlescriptCurrInstr++;
                    }
...
static void atk88_negativedamage(void)
{
    ...
    if (gBattleMoveDamage == 0)
        gBattleMoveDamage = -1;
    if (ItemId_GetHoldEffect(gBattleMons[gBankAttacker].item) == HOLD_EFFECT_BOOST_DRAIN_MOVES)
