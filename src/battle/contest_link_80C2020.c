    ...
    switch (gTasks[taskId].data[0])
    {
        case 0:
            ...
            if (GetGenderFromSpeciesAndPersonality(species, personality) == MON_FEMALE)
                HandleLoadSpecialPokePic(gMonFrontPicTableFemale + species, gMonFrontPicCoords[species].coords, gMonFrontPicCoords[species].y_offset, (intptr_t)gSharedMem, gUnknown_081FAF4C[1], species, personality);
            else
