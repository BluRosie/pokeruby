...
void HandleAction_UseMove(void)
{
        ...
        && gBattleMons[gSideTimers[side].followmeTarget].hp != 0)
    {
        gBattlerTarget = gSideTimers[side].followmeTarget;
             ...
             && ((GetBattlerAbility(gSharedMem[BSTRUCT_OFF(moveTarget) + gBattlerAttacker]) != ABILITY_LIGHTNING_ROD
                  && gBattleMoves[gCurrentMove].type == TYPE_ELECTRIC)
              || (GetBattlerAbility(gSharedMem[BSTRUCT_OFF(moveTarget) + gBattlerAttacker]) != ABILITY_STORM_DRAIN
