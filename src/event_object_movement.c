...
static void InitFollower(struct EventObject *);
...
const u8 gInitialMovementTypeFacingDirections[] = {
    ...
    DIR_NORTH, // MOVEMENT_TYPE_FOLLOWER
};
...
static bool8 IsCoordOutsideEventObjectMovementRange(struct EventObject *eventObject, s16 x, s16 y)
{
    ...
    if (eventObject->movementType == MOVEMENT_TYPE_FOLLOWER)
    {
        return FALSE;
    }
    if (eventObject->range.as_nybbles.x != 0)
...
static bool8 DoesObjectCollideWithObjectAt(struct EventObject *eventObject, s16 x, s16 y)
{
    ...
    for (i = 0; i < EVENT_OBJECTS_COUNT; i++)
    {
        ...
        {
            if (((eventObject2->currentCoords.x == x && eventObject2->currentCoords.y == y) // if the destination coords are the same as an npc
                || (eventObject2->previousCoords.x == x && eventObject2->previousCoords.y == y && eventObject->movementType != MOVEMENT_TYPE_FOLLOWER)) // if the previous coords are the same as an npc and the one in question isn't the follower (this one for the follower specifically)
                && AreZCoordsCompatible(eventObject->currentElevation, eventObject2->currentElevation) // if they're on the same height
                && eventObject2->movementType != MOVEMENT_TYPE_FOLLOWER) // if an npc blocks the way that isn't the follower
                return TRUE;
        }
    }
    return FALSE;
...
void ledge_helper(struct EventObject *eventObject, struct Sprite *sprite, u8 direction, u8 a4, u8 a5)
{
    s16 vSPp4[3];
    s16 x;
    s16 y;
...
void DoLedge(struct EventObject *eventObject, struct Sprite *sprite, u8 direction, u8 speed, u8 a5)
{
    ledge_helper(eventObject, sprite, direction, speed, a5);
    sub_805FE28(eventObject, sprite, GetMoveDirectionAnimNum(eventObject->facingDirection));
    DoShadowFieldEffect(eventObject);
}
...
u8 is_jump3_finished(struct EventObject *eventObject, struct Sprite *sprite, u8 (*const callback)(struct Sprite *))
{
    s16 vSPp4[3];
    s16 x;
    s16 y;
...
bool8 IsJumpFinished(struct EventObject *eventObject, struct Sprite *sprite)
{
...
{
    if (IsJumpFinished(eventObject, sprite))
    {
        eventObject->hasShadow = FALSE;
        sprite->data[2] = 2;
        return TRUE;
...
{
    DoLedge(eventObject, sprite, DIR_SOUTH, 1, 2);
...
bool8 MovementAction_Jump3Down_Step1(struct EventObject *, struct Sprite *);
bool8 MovementAction_Jump3Up_Step1(struct EventObject *, struct Sprite *);
bool8 MovementAction_Jump3Left_Step1(struct EventObject *, struct Sprite *);
bool8 MovementAction_Jump3Right_Step1(struct EventObject *, struct Sprite *);

bool8 MovementAction_Jump3Down_Step0(struct EventObject *eventObject, struct Sprite *sprite)
{
    DoLedge(eventObject, sprite, DIR_SOUTH, 2, 0);
    return MovementAction_Jump3Down_Step1(eventObject, sprite);
}

bool8 MovementAction_Jump3Down_Step1(struct EventObject *eventObject, struct Sprite *sprite)
{
    if (is_jump3_finished(eventObject, sprite, move_follower_for_jump) == 0xFF)
    {
        eventObject->hasShadow = FALSE;
        sprite->data[2] = 2;
        return TRUE;
...
bool8 MovementAction_Jump3Up_Step0(struct EventObject *eventObject, struct Sprite *sprite)
{
    DoLedge(eventObject, sprite, DIR_NORTH, 2, 0);
    return MovementAction_Jump3Up_Step1(eventObject, sprite);
}

bool8 MovementAction_Jump3Up_Step1(struct EventObject *eventObject, struct Sprite *sprite)
{
    if (is_jump3_finished(eventObject, sprite, move_follower_for_jump) == 0xFF)
    {
        eventObject->hasShadow = FALSE;
...
bool8 MovementAction_Jump3Left_Step0(struct EventObject *eventObject, struct Sprite *sprite)
{
    DoLedge(eventObject, sprite, DIR_WEST, 2, 0);
    return MovementAction_Jump3Left_Step1(eventObject, sprite);
}

bool8 MovementAction_Jump3Left_Step1(struct EventObject *eventObject, struct Sprite *sprite)
{
    if (is_jump3_finished(eventObject, sprite, move_follower_for_jump) == 0xFF)
    {
        eventObject->hasShadow = FALSE;
...
bool8 MovementAction_Jump3Right_Step0(struct EventObject *eventObject, struct Sprite *sprite)
{
    DoLedge(eventObject, sprite, DIR_EAST, 2, 0);
    return MovementAction_Jump3Right_Step1(eventObject, sprite);
}

bool8 MovementAction_Jump3Right_Step1(struct EventObject *eventObject, struct Sprite *sprite)
{
    if (is_jump3_finished(eventObject, sprite, move_follower_for_jump) == 0xFF)
    {
        eventObject->hasShadow = FALSE;
...
static const s16 gStepSpeed_Seq_Length[] = {
    16, 8, 6, 4, 2
};

bool8 obj_npc_ministep(struct Sprite *sprite)
{
    if (sprite->data[5] >= gStepSpeed_Seq_Length[sprite->data[4]])
        return FALSE;
...
//data[3] - delta pos/direction?
//data[4] - some sort of state chooser
//data[5] - not one bit of clue
//data[6] - currFrame

u8 move_follower_for_jump(struct Sprite *sprite)
{
    s16 ledge_frames_table[3] = {0x10, 0x10, 0x20};
    u8 v6[3] = {0, 0, 1};
    u8 v2 = 0;
    ...
    if (sprite->data[6] == (ledge_frames_table[sprite->data[4]] >> 1))
    {
        v2 = 1;
    }

    if (sprite->data[6] >= ledge_frames_table[sprite->data[4]])
    {
        sprite->pos2.y = 0;
        v2 = -1;
...
u8 GetFollowerGraphicsId(void)
{
    return gFollowerStruct->graphicsId;
}

void SetFollowerGraphicsId(u8 id)
{
    gFollowerStruct->graphicsId = id;
}
...
{
    gFollowerStruct->cantMove = 0;
}

void StopFollower(void)
{
    gFollowerStruct->cantMove = 1;
}
...
bool8 MovementType_Follower_Step0(struct EventObject *eventObject, struct Sprite *sprite)
{
    if (!gFollowerStruct->init)
        InitFollower(eventObject);
    ClearEventObjectMovement(eventObject, sprite);
    if (eventObject->directionSequenceIndex == 0)
    {
        eventObject->directionSequenceIndex = GetPlayerMovementDirection();
    }
    sprite->data[1] = 1;
    return TRUE;
}
...
bool8 MovementType_Follower_Step1(struct EventObject *eventObject, struct Sprite *sprite)
{
    ...
    {
        return gFollowerMovementFuncs[PlayerGetCopyableMovement()](eventObject, sprite, gFollowerStruct->currDir, NULL);
    }
}
...
bool8 FollowerMovement_FaceDirection(struct EventObject *eventObject, struct Sprite *sprite, u8 playerDirection, bool8 tileCallback(u8))
{
    if (playerDirection == 0)
        return FALSE;
    EventObjectSetSingleMovement(eventObject, sprite, GetFaceDirectionMovementAction(playerDirection));
    eventObject->singleMovementActive = 1;
    sprite->data[1] = 2;
    return TRUE;
}

bool8 FollowerMovement_GoSpeed0(struct EventObject *eventObject, struct Sprite *sprite, u8 playerDirection, bool8 tileCallback(u8))
{
    u32 direction;
    s16 x;
    s16 y;
...
bool8 FollowerMovement_GoSpeed1(struct EventObject *eventObject, struct Sprite *sprite, u8 playerDirection, bool8 tileCallback(u8))
{
    u32 direction;
    s16 x;
    s16 y;
    ...
    if (playerDirection == 0)
        return FALSE;
    direction = gFollowerStruct->currDir - 5;
    EventObjectMoveDestCoords(eventObject, direction, &x, &y);
    EventObjectSetSingleMovement(eventObject, sprite, GetWalkFastMovementAction(direction));
    if (GetCollisionAtCoords(eventObject, x, y, direction) || (tileCallback != NULL && !tileCallback(MapGridGetMetatileBehaviorAt(x, y))))
    {
...
bool8 FollowerMovement_GoSpeed2(struct EventObject *eventObject, struct Sprite *sprite, u8 playerDirection, bool8 tileCallback(u8))
{
    u32 direction;
    s16 x;
    s16 y;
...
bool8 FollowerMovement_Slide(struct EventObject *eventObject, struct Sprite *sprite, u8 playerDirection, bool8 tileCallback(u8))
{
    u32 direction;
    s16 x;
    s16 y;
...
bool8 follower_IM_DIFFERENT(struct EventObject *eventObject, struct Sprite *sprite, u8 playerDirection, bool8 tileCallback(u8))
{
    u32 direction;
...
bool8 FollowerMovement_GoSpeed4(struct EventObject *eventObject, struct Sprite *sprite, u8 playerDirection, bool8 tileCallback(u8))
{
    u32 direction;
    s16 x;
    s16 y;
...
bool8 FollowerMovement_Jump(struct EventObject *eventObject, struct Sprite *sprite, u8 playerDirection, bool8 tileCallback(u8))
{
    u32 direction;
    s16 x;
    s16 y;
    ...
    if (playerDirection == 5)
        direction = gFollowerStruct->ledgeDirection;
    else
        direction = GetPlayerFacingDirection();
    x = eventObject->currentCoords.x;
    y = eventObject->currentCoords.y;
    MoveCoordsInDirection(direction, &x, &y, 2, 2);
    EventObjectSetSingleMovement(eventObject, sprite, GetJump3MovementAction(direction));
    if (GetCollisionAtCoords(eventObject, x, y, direction) || (tileCallback != NULL && !tileCallback(MapGridGetMetatileBehaviorAt(x, y))))
    {
        EventObjectSetSingleMovement(eventObject, sprite, GetFaceDirectionMovementAction(direction));
...
bool8 FollowerMovement_DelayedPrep(struct EventObject *eventObject, struct Sprite *sprite, u8 playerDirection, bool8 tileCallback(u8))
{
    u32 direction;
    s16 x;
    s16 y;
