    ...
    if (gPokemonStorageSystemPtr->unk_2700)
    {
        if (species != SPECIES_NONE)
        {
            if (GetGenderFromSpeciesAndPersonality(species, pid) == MON_FEMALE)
                HandleLoadSpecialPokePic(gMonFrontPicTableFemale + species, gMonFrontPicCoords[species].coords, 1, gPokemonStorageSystemPtr->unk_4784, gPokemonStorageSystemPtr->unk_2784, species, pid);
            else
